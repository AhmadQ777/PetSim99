pkg update -y && pkg upgrade -y && pkg install python curl -y && pip install requests && termux-setup-storage && termux-wake-lock && mkdir -p ~/PetSim99 && mkdir -p /storage/emulated/0/Delta/Autoexecute && mkdir -p /storage/emulated/0/Delta/Workspace && cd ~/PetSim99 && 
...
# ---------------- CONFIG DOWNLOAD ----------------
curl -s --fail --max-time 10 "https://raw.githubusercontent.com/AhmadQ777/PetSim99/main/Data/Config.json" -o Config.json || { sleep 180; continue; }
